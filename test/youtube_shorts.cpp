...
static const int WINDOW_SIZE_X = 720, WINDOW_SIZE_Y = 1280;
...
cv::Mat sfmlToCv(const sf::Image& image) {
    sf::Vector2u size = image.getSize();
    cv::Mat mat(size.y, size.x, CV_8UC4, (void*)image.getPixelsPtr());
    cv::Mat matBGR;
...
int main(int argc, char **argv) {
    ...
    view.setSize(2.0f, -2.0f * ((float)WINDOW_SIZE_Y / (float)WINDOW_SIZE_X));
    view.setViewport(sf::FloatRect(0, 0, 1, 1));

    win.setView(view);

    // test a cirlce body made of triangle volume conservations
    createCircle(system, 0.2f, 30, 0.015f, 0.0f);

    unsigned long long frameCount = 0; 

    cv::VideoWriter videoWriter;
    int codec = cv::VideoWriter::fourcc('M', 'J', 'P', 'G'); // Codec
    int fps = 60; // Frames per second
    videoWriter.open("output.avi", codec, fps, cv::Size(WINDOW_SIZE_X, WINDOW_SIZE_Y));

    if (!videoWriter.isOpened()) {
        std::cerr << "Error: Could not open the video file for write.\n";
        return -1;
    }
    ...
    while (win.isOpen()) {
        ...
        sf::CircleShape border(0.9f, 120);
        border.setPosition(0.0f - 0.9f, 0.0f - 0.9f);
        ...
        for (auto& particle : system.particles) {
            ...
            glm::vec2 uv = particle->position - particle->radius;

            circle.setPosition(uv.x, uv.y);
        ...
        cv::Mat frame = sfmlToCv(screenshot);
